...
Variants {
    ...
    delegate: Component {
		...
		"quickactions/Timer.qml"
            ...
            // Event-driven fullscreen detection: hide edge widget when the
            // focused window is fullscreen (same mechanism as TopBar)
            Process { id: fsWaiter; command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/fullscreen_wait.sh"]; onExited: { fsPoller.running = false; fsPoller.running = true; } }
            Process {
                ...
                stdout: StdioCollector {
                    onStreamFinished: {
                        ...
                        if (fs !== floatingWidget.isFullscreen) {
                            floatingWidget.isFullscreen = fs;
                            if (fs) floatingWidget.isExpanded = false;
                        }
                    }
                }
            }
