...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        function immediateRgbSync(imagePath) {
            return `
                python3 - <<'PYRGB' "${escapeBash(imagePath)}" 2>/dev/null || true
import sys
...
try:
    ...
    r2,g2,b2 = colorsys.hls_to_rgb(h, 0.45, 0.90)
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    IMMEDIATE_COLOR=$(python3 - <<'PYRGB' "$DEST_FILE" 2>/dev/null || true
import sys
from PIL import Image
import colorsys
path = sys.argv[1]
...
PYRGB
                    ...
                    if [ -n "$IMMEDIATE_COLOR" ] && [ "$IMMEDIATE_COLOR" != "#000000" ]; then
                        python3 "$HOME/Dotfiles/scripts/fade-rgb.py" "$IMMEDIATE_COLOR" 70 70 >/dev/null 2>&1 &
                        python3 "$HOME/Dotfiles/scripts/govee-led.py" "$IMMEDIATE_COLOR" 70 --fade >/dev/null 2>&1 || true
                    fi
...
PYRGB
        ...
        if (isVideo) {
            ...
            wallpaperCmd = `
                ...
                IMMEDIATE_COLOR=$(python3 - <<'PYRGB' "${escOriginal}" 2>/dev/null || true
import sys
from PIL import Image
import colorsys
path = sys.argv[1]
...
PYRGB
    ...
    Shortcut { sequence: "Escape"; enabled: !window.isApplying; onActivated: {
        // Clear an active search first; otherwise close the wallpaper picker.
        if (window.currentFilter === "Search") {
