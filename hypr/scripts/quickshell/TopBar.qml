...
Variants {
    ...
    delegate: Component {
		...
		running: true
            ...
            Process { id: fullscreenWaiter; command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/fullscreen_wait.sh"]; onExited: { fullscreenPoller.running = false; fullscreenPoller.running = true; } }
            Process {
                id: fullscreenPoller; running: true
                command: ["bash", "-c", "hyprctl activewindow -j 2>/dev/null | jq -r '.fullscreen // 0'"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let fs = parseInt(this.text.trim()) >= 1;
                        barWindow.isFullscreen = fs;
                    }
                }
            }
            // Event-driven via fullscreen_wait.sh socket watcher; no timer needed
            ...
            Item {
                ...
                Rectangle {
                    ...
                    color: barWindow.isFullscreen ? "transparent" : Qt.rgba(mocha.base.r, mocha.base.g, mocha.base.b, 0.75)
                    radius: barWindow.s(14)
                    border.width: barWindow.isFullscreen ? 0 : 1
                    border.color: Qt.rgba(mocha.text.r, mocha.text.g, mocha.text.b, 0.08)
                    clip: true
                    
                    property bool showLayout: false
                    
                    opacity: barWindow.isFullscreen ? 0 : (showLayout && !barWindow.isSettingsOpen) ? 1 : 0
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        Row {
                            ...
                            Rectangle {
                                id: coffeePill
                                property bool isHovered: coffeeMouse.containsMouse
                                color: isHovered ? Qt.rgba(mocha.surface1.r, mocha.surface1.g, mocha.surface1.b, 0.6) : 'transparent'
                                radius: barWindow.s(10)
                                height: sysLayout.pillHeight; width: barWindow.s(34)
                                visible: barWindow.stayActive
                                Behavior on color { ColorAnimation { duration: 200 } }
                                ...
                                MouseArea {
                                    ...
                                    anchors.fill: parent
                                    hoverEnabled: true
                                    onClicked: Quickshell.execDetached(['bash', '-c', '~/.config/hypr/scripts/qs_manager.sh toggle stay-active'])
                                }
                            }
