...
Item {
    ...
    // props injected by Main.qml/executeSwitch
    ...
    function isCommandQuery(q) {
        return q.startsWith(">");
    }

    function getCommandQuery(q) {
    ...
    function filterApps(query) {
        ...
        window.commandMode = isCommandQuery(query);
        
        if (window.commandMode) {
            // Command mode: filter commands
            let cmdQ = getCommandQuery(query);
            let filtered = [];
            
            // Add recent commands
            for (let i = 0; i < window.commandHistory.length; i++) {
                if (window.commandHistory[i].toLowerCase().includes(cmdQ.toLowerCase())) {
                    filtered.push({name: "> " + window.commandHistory[i], exec: window.commandHistory[i], icon: "utilities-terminal"});
                }
            }
            
            // Add common commands
            let common = [
                {name: "> hyprctl reload", exec: "hyprctl reload", icon: "preferences-system"},
                {name: "> killall quickshell", exec: "killall quickshell", icon: "application-exit"},
                {name: "> systemctl reboot", exec: "systemctl reboot", icon: "system-reboot"},
                {name: "> systemctl poweroff", exec: "systemctl poweroff", icon: "system-shutdown"},
            ];
            for (let i = 0; i < common.length; i++) {
                if (common[i].name.toLowerCase().includes(cmdQ.toLowerCase()) && filtered.length < 8) {
                    filtered.push(common[i]);
                }
            }
    ...
    function launchApp(execStr) {
        if (window.commandMode) {
            ...
            window.commandHistory.unshift(execStr);
            if (window.commandHistory.length > 20) window.commandHistory.pop();
    ...
    Keys.onPressed: event => {
        if (event.key === Qt.Key_PageUp) {
            if (appList.currentIndex > 0) {
                appList.currentIndex = Math.max(0, appList.currentIndex - Math.max(1, Math.floor(appList.height / appList.contentHeight * appList.count)));
                event.accepted = true;
            }
        } else if (event.key === Qt.Key_PageDown) {
            if (appList.currentIndex < appModel.count - 1) {
                appList.currentIndex = Math.min(appModel.count - 1, appList.currentIndex + Math.max(1, Math.floor(appList.height / appList.contentHeight * appList.count)));
                event.accepted = true;
            }
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    TextField {
                        ...
                        onTextChanged: {
                            filterApps(text);
                            searchIcon.text = window.commandMode ? "" : "";
                        }
                        ...
                        // Left margin when in command mode
                        Rectangle {
                            anchors.verticalCenter: parent.verticalCenter
                            anchors.left: parent.left
                            width: window.commandMode ? window.s(24) : 0
                            height: 1
                            color: "transparent"
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            Text {
                                ...
                                transform: Translate { x: index === appList.currentIndex ? window.s(6) : 0 }
